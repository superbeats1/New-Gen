...
const SentimentGauge: React.FC<{ value: number; label: string }> = ({ value, label }) => {
  ...
  return (
    <div className="relative w-28 h-14 sm:w-48 sm:h-24 overflow-hidden mb-2 sm:mb-4">
      <div className="absolute inset-0 border-[8px] sm:border-[12px] border-slate-800 rounded-t-full"></div>
      <div
        className="absolute inset-0 border-[8px] sm:border-[12px] border-violet-500 rounded-t-full transition-all duration-1000 ease-out"
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center">
        ...
        <div className="text-[7px] sm:text-[10px] text-slate-500 uppercase tracking-wider sm:tracking-widest font-bold">{label}</div>
      </div>
      <div
        className="absolute bottom-2 sm:bottom-4 left-1/2 w-0.5 sm:w-1 h-8 sm:h-12 bg-white/80 origin-bottom transform transition-transform duration-1000"
...
const OpportunityCard: React.FC<{ opportunity: Opportunity; index: number }> = ({ opportunity, index }) => {
  ...
  return (
    <div className={`group bg-[#050608]/40 backdrop-blur-2xl rounded-2xl sm:rounded-[3rem] overflow-hidden transition-all duration-700 border border-white/10 hover:border-violet-500/50 ${isExpanded ? 'shadow-[0_0_80px_rgba(139,92,246,0.15)] ring-1 ring-violet-500/20' : ''} relative`}>
      ...
      <div
        className="p-4 sm:p-8 lg:p-12 cursor-pointer flex items-start justify-between relative z-10"
        onClick={(e) => {
          e.stopPropagation();
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 mb-4 sm:mb-6">
            <div className="bg-violet-600/20 border border-violet-500/30 px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-full flex items-center space-x-1.5 sm:space-x-2">
              <Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-violet-400 fill-violet-400 flex-shrink-0" />
              <span className="text-[9px] sm:text-[10px] font-black text-white uppercase tracking-wider sm:tracking-widest italic">Signal Alpha-{index + 1}</span>
            </div>
            <div className="hidden sm:block h-1.5 w-1.5 rounded-full bg-slate-700"></div>
            <span className={`text-[9px] sm:text-[11px] font-black uppercase tracking-wider sm:tracking-widest ${getScoreColor(opportunity.overallScore)}`}>
              Integrity: {opportunity.overallScore * 10}%
            </span>
          </div>

          {/* SPEED TO $1K MRR - UNIQUE COMPETITIVE ADVANTAGE */}
          {opportunity.speedToMRR && (
            <div className={`mb-4 sm:mb-6 px-3 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl border-2 backdrop-blur-md flex flex-col sm:flex-row sm:inline-flex sm:items-center gap-3 sm:space-x-4 ${
              ...
              <div className="flex items-center space-x-2">
                ...
                {opportunity.speedToMRR.velocity === 'Slow' && (
                  <svg className="w-4 h-4 sm:w-5 sm:h-5 text-slate-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v3.586L7.707 9.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 10.586V7z" clipRule="evenodd" />
                  </svg>
                )}
                <div>
                  <div className={`text-[8px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-widest ${
                    opportunity.speedToMRR.velocity === 'Fast'
                      ? 'text-emerald-400'
                      : opportunity.speedToMRR.velocity === 'Medium'
                      ? 'text-amber-400'
                  ...
                  <div className="text-base sm:text-xl font-black text-white tracking-tight">
                    {opportunity.speedToMRR.estimatedWeeks} wks to $1K MRR
                  </div>
                </div>
              </div>
              <div className="hidden sm:block h-10 w-px bg-white/10"></div>
              <div className="text-[10px] sm:text-xs text-slate-400 sm:max-w-xs font-medium leading-tight">
                {opportunity.speedToMRR.reasoning}
              </div>
            </div>
          )}

          <h3 className="text-xl sm:text-3xl lg:text-5xl font-black text-white mb-4 sm:mb-8 leading-[1.1] tracking-tighter group-hover:text-violet-100 transition-colors">
            {opportunity.problemStatement}
          </h3>

          <div className="flex flex-wrap gap-2 sm:gap-4">
            <div className="flex flex-col bg-white/[0.03] px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl border border-white/5 backdrop-blur-md">
              <span className="text-[7px] sm:text-[9px] text-slate-500 uppercase tracking-wider sm:tracking-widest font-black mb-0.5 sm:mb-1">Market Flux</span>
              <span className={`text-sm sm:text-lg font-black ${getScoreColor(opportunity.overallScore)}`}>{opportunity.overallScore}/10</span>
            </div>
            {opportunity.confidenceLevel && (
              <div className="flex flex-col bg-violet-600/10 px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl border border-violet-500/20 backdrop-blur-md">
                <span className="text-[7px] sm:text-[9px] text-violet-400 uppercase tracking-wider sm:tracking-widest font-black mb-0.5 sm:mb-1">Confidence</span>
                <span className="text-sm sm:text-lg font-black text-white">{opportunity.confidenceLevel}/10</span>
              </div>
            )}
            {opportunity.demandSubMetrics && (
              <div className="flex flex-col bg-white/[0.03] px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl border border-white/5 backdrop-blur-md">
                <span className="text-[7px] sm:text-[9px] text-slate-500 uppercase tracking-wider sm:tracking-widest font-black mb-0.5 sm:mb-1">Intensity</span>
                <span className="text-sm sm:text-lg font-black text-indigo-400">{opportunity.demandSubMetrics.intensity}/10</span>
              </div>
            )}
          </div>
        </div>
        <div className="flex flex-col items-center space-y-2 sm:space-y-4 ml-2 sm:ml-0">
          <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-violet-500/50 transition-all flex-shrink-0 ${isExpanded ? 'bg-violet-600/10' : ''}`}>
            {isExpanded ? <ChevronUp className="w-4 h-4 sm:w-6 sm:h-6 text-white" /> : <ChevronDown className="w-4 h-4 sm:w-6 sm:h-6 text-slate-400" />}
          </div>
        </div>
      </div>

      {isExpanded && (
        <div className="px-4 sm:px-5 lg:px-8 pb-6 sm:pb-8 lg:pb-10 border-t border-white/5 pt-4 sm:pt-6 lg:pt-8 animate-in slide-in-from-top-2 duration-300">
          {opportunity.trendData && (
            <div className="mb-6 sm:mb-10 p-4 sm:p-10 rounded-xl sm:rounded-[2.5rem] bg-gradient-to-br from-violet-600/10 via-indigo-600/5 to-transparent border border-violet-500/20 relative overflow-hidden group/trend">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-8 gap-2">
                <h4 className="text-[10px] sm:text-xs font-black text-violet-400 uppercase tracking-[0.2em] sm:tracking-[0.3em]">Neural Trend Analysis</h4>
                <div className="flex items-center space-x-2 text-[9px] sm:text-[10px] text-slate-500 font-bold uppercase tracking-wider sm:tracking-widest">
                  <Activity className="w-2.5 h-2.5 sm:w-3 sm:h-3 flex-shrink-0" />
                  <span>Real-time Flux</span>
                </div>
              </div>

              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-10 relative z-10">
                <div className="space-y-1 sm:space-y-2">
                  <div className="text-[8px] sm:text-[10px] text-slate-500 uppercase tracking-wider sm:tracking-widest font-black">Search Intensity</div>
                  <div className="text-lg sm:text-3xl font-black text-white group-hover/trend:text-violet-200 transition-colors uppercase italic tracking-tighter">{opportunity.trendData.searchVolume}</div>
                </div>
                <div className="space-y-1 sm:space-y-2">
                  <div className="text-[8px] sm:text-[10px] text-slate-500 uppercase tracking-wider sm:tracking-widest font-black">Growth Momentum</div>
                  <div className={`text-lg sm:text-3xl font-black flex items-center ${opportunity.trendData.trend === 'rising' ? 'text-emerald-400' : 'text-slate-400'} italic tracking-tighter`}>
                    {opportunity.trendData.growthRate}
                    {opportunity.trendData.trend === 'rising' && <TrendingUp className="w-3.5 h-3.5 sm:w-5 sm:h-5 ml-1 sm:ml-2 flex-shrink-0" />}
                  </div>
                </div>
                <div className="flex items-center col-span-2 lg:col-span-1 mt-2 lg:mt-0">
                  <div className={`px-3 sm:px-6 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-[8px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-[0.2em] shadow-lg ${opportunity.trendData.trend === 'rising' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-slate-500/20 text-slate-400 border border-slate-500/30'}`}>
...
const OpportunityView: React.FC<Props> = ({ results, onNewSearch }) => {
  ...
  return (
    <div className="space-y-6 lg:space-y-10 w-full overflow-hidden">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-6">
        <div className="min-w-0 flex-1">
          <div className="flex items-center space-x-2 text-slate-500 text-xs lg:text-sm mb-2 font-medium">
            ...
            <span className="uppercase tracking-[0.15em] sm:tracking-[0.2em] text-[9px] sm:text-[10px] font-black text-emerald-400/80">Analysis Protocol Active</span>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-baseline sm:space-x-4">
            <h2 className="text-3xl sm:text-4xl lg:text-7xl font-black text-white tracking-tighter uppercase italic">Market Report</h2>
            {results.dateRange && (
              <span className="text-[10px] sm:text-xs font-black text-slate-500 uppercase tracking-widest bg-white/5 px-3 py-1 rounded-full border border-white/10 mt-2 sm:mt-0 w-fit">
                {results.dateRange}
              </span>
            )}
          </div>
          {results.queryInterpretation && (
            <div className="mt-4 p-3 sm:p-4 bg-violet-600/5 border border-violet-500/10 rounded-xl sm:rounded-2xl">
              <span className="text-[9px] font-black text-violet-400 uppercase tracking-widest block mb-1">Intelligence Context</span>
              <p className="text-slate-400 text-xs sm:text-sm italic break-words">"{results.queryInterpretation}"</p>
            </div>
          )}
          <div className="grid grid-cols-3 gap-3 sm:flex sm:items-center sm:space-x-6 mt-4 sm:mt-6">
            <div className="flex flex-col min-w-0">
              <span className="text-[8px] sm:text-[9px] text-slate-500 uppercase tracking-widest font-black mb-1 truncate">Sources Scanned</span>
              <span className="text-lg sm:text-2xl font-black text-white">{results.totalSourcesAnalyzed || 0}</span>
            </div>
            <div className="hidden sm:block w-[1px] h-8 bg-white/10"></div>
            <div className="flex flex-col min-w-0">
              <span className="text-[8px] sm:text-[9px] text-slate-500 uppercase tracking-widest font-black mb-1 truncate">Query Resolution</span>
              <span className={`text-lg sm:text-2xl font-black ${(results.opportunities?.length || 0) > 0 ? 'text-emerald-400' : 'text-amber-400'}`}>
                {(results.opportunities?.length || 0) > 0 ? 'Stable' : 'Degraded'}
              </span>
            </div>
            <div className="hidden sm:block w-[1px] h-8 bg-white/10"></div>
            <div className="flex flex-col min-w-0">
              <span className="text-[8px] sm:text-[9px] text-slate-500 uppercase tracking-widest font-black mb-1 truncate">Signal Alpha</span>
              <span className="text-lg sm:text-2xl font-black text-violet-400">{results.opportunities?.length || 0}</span>
            </div>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full lg:w-auto">
          <div ref={exportMenuRef} className="relative flex-1 lg:flex-none">
            <button
              ...
              className="w-full flex items-center justify-center space-x-2 sm:space-x-3 bg-white/5 hover:bg-white/10 px-4 sm:px-8 py-3 sm:py-5 rounded-xl sm:rounded-[1.5rem] text-slate-400 transition-all text-[9px] sm:text-[10px] font-black uppercase tracking-widest border border-white/5 group"
            >
              <Download className="w-3.5 h-3.5 sm:w-4 sm:h-4 group-hover:translate-y-0.5 transition-transform flex-shrink-0" />
              <span className="truncate">Export Intelligence</span>
              <ChevronDown className={`w-3 h-3 transition-transform flex-shrink-0 ${showExportMenu ? 'rotate-180' : ''}`} />
            </button>
          ...
          <button
            onClick={onNewSearch}
            className="flex-1 lg:flex-none flex items-center justify-center space-x-2 sm:space-x-3 bg-violet-600 hover:bg-violet-500 px-4 sm:px-10 py-3 sm:py-5 rounded-xl sm:rounded-[1.5rem] text-white transition-all text-[9px] sm:text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-violet-600/30 group"
          >
            <Target className="w-3.5 h-3.5 sm:w-4 sm:h-4 group-hover:scale-110 transition-transform flex-shrink-0" />
            <span className="truncate">New Neuro-Scan</span>
          </button>
        </div>
      </div>

      {/* Market Pulse Ticker */}
      <div className="w-full h-9 sm:h-11 bg-white/[0.02] border-y border-white/5 overflow-hidden flex items-center whitespace-nowrap relative">
        <div className="absolute left-0 top-0 bottom-0 w-8 sm:w-20 bg-gradient-to-r from-[#030407] to-transparent z-10"></div>
        <div className="absolute right-0 top-0 bottom-0 w-8 sm:w-20 bg-gradient-to-l from-[#030407] to-transparent z-10"></div>
        <div className="animate-ticker flex items-center space-x-6 sm:space-x-12 px-6 sm:px-12">
          {[1, 2, 3].map(i => (
            <React.Fragment key={i}>
              <span className="flex items-center space-x-1.5 sm:space-x-2 text-[8px] sm:text-[10px] font-bold text-slate-500 uppercase tracking-wider sm:tracking-widest">
                <Activity className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-violet-500 flex-shrink-0" />
                <span>Protocol 17.4x</span>
              </span>
              <span className="flex items-center space-x-1.5 sm:space-x-2 text-[8px] sm:text-[10px] font-bold text-slate-500 uppercase tracking-wider sm:tracking-widest">
                <TrendingUp className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-emerald-500 flex-shrink-0" />
                <span>Demand +12.4%</span>
              </span>
              <span className="flex items-center space-x-1.5 sm:space-x-2 text-[8px] sm:text-[10px] font-bold text-slate-500 uppercase tracking-wider sm:tracking-widest">
                <Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-amber-500 flex-shrink-0" />
                <span>Signal Alpha-{Math.floor(Math.random() * 900) + 100}</span>
              </span>
              <span className="hidden sm:flex items-center space-x-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
        <div className="p-4 sm:p-8 bg-[#050608]/40 backdrop-blur-2xl rounded-2xl sm:rounded-[2.5rem] border border-white/10 group hover:border-violet-500/30 transition-all relative overflow-hidden">
          ...
          <span className="text-slate-500 text-[8px] sm:text-[10px] font-black uppercase tracking-[0.1em] sm:tracking-[0.2em] mb-2 sm:mb-4 block">Intelligence Index</span>
          <div className="text-3xl sm:text-6xl font-black text-white mb-2 sm:mb-4 italic tracking-tighter">48.2</div>
          <div className="text-[8px] sm:text-[10px] text-emerald-400 font-black uppercase tracking-wider sm:tracking-widest flex items-center bg-emerald-400/10 w-fit px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border border-emerald-500/20">
            <TrendingUp className="w-2.5 h-2.5 sm:w-3 sm:h-3 mr-1 sm:mr-2 flex-shrink-0" /> <span className="truncate">High Efficiency</span>
          </div>
        </div>

        <div className="p-4 sm:p-8 bg-[#050608]/40 backdrop-blur-2xl rounded-2xl sm:rounded-[2.5rem] border border-white/10 group hover:border-violet-500/30 transition-all flex flex-col items-center justify-center">
          <SentimentGauge value={results.opportunities?.[0]?.marketSentiment || 72} label="Market Intensity" />
        </div>

        <div className="p-4 sm:p-8 bg-[#050608]/40 backdrop-blur-2xl rounded-2xl sm:rounded-[2.5rem] border border-white/10 group hover:border-violet-500/30 transition-all">
          <span className="text-slate-500 text-[8px] sm:text-[10px] font-black uppercase tracking-[0.1em] sm:tracking-[0.2em] mb-2 sm:mb-4 block">Growth Velocity</span>
          <div className="text-3xl sm:text-5xl font-black text-white mb-2 sm:mb-4 italic tracking-tighter">
        ...
        <div className="p-4 sm:p-8 bg-[#050608]/40 backdrop-blur-2xl rounded-2xl sm:rounded-[2.5rem] border border-white/10 group hover:border-violet-500/30 transition-all relative overflow-hidden">
          ...
          <div className="text-2xl sm:text-4xl font-black text-white mb-2 sm:mb-4 italic tracking-tighter uppercase">Optimized</div>
          <div className="text-[8px] sm:text-[10px] text-violet-400 font-black uppercase tracking-wider sm:tracking-widest bg-violet-400/10 w-fit px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border border-violet-500/20">
            <span className="truncate">{results.opportunities?.[0]?.marketMaturity || 'Emerging'} Stage</span>
          </div>
        </div>
      </div>
